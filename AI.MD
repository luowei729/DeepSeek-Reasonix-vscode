...
1.请优先使用 semantic_search 进行向量索引来检索，读到或没读到都要中文提示我。 
2.按照项目代码功能结构,功能区域划分规则，进行开发修改，不要擅自改变代码架构和功能划分结构。 
3.你先规划架构，不明白的细节可以先和我沟通确认，再写代码，你开发阶段可以打开 无头 chrome 访问主站页面调试验证。 
4.每次写代码前先给“改动前总结”，写完后给“改动后总结”，每次更改变动要检查修改写入项目根目录的AI.MD记录写入日期和时间，方便后期再开发修改的时候快速定位 
5.你开发阶段可以打开 无头 chrome 实时查看页面调试验证你的修改没问题，验证电脑和手机及mac电脑页面显示没问题风格符合整体UI，再结束 
6.要求代码里每步都要注释功能的实现和实现的原因，为后期排查问题和开发做好基础
